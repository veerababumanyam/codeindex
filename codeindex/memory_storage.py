...
async def fts5_available(conn: aiosqlite.Connection) -> bool:
    if os.getenv("CODEINDEX_DISABLE_FTS5", "").lower() in {"1", "true", "yes", "on"}:
        return False
    try:
        await conn.execute("CREATE VIRTUAL TABLE IF NOT EXISTS __memory_fts_probe USING fts5(value)")
        await conn.execute("DROP TABLE IF EXISTS __memory_fts_probe")
        return True
...
class MemoryStorage:
    ...
    @classmethod
    async def create(cls, conn: aiosqlite.Connection) -> MemoryStorage:
        storage = cls(conn)
        await storage.conn.executescript(BASE_MEMORY_SCHEMA)
        storage._fts5_enabled = await fts5_available(conn)
        if storage._fts5_enabled:
            await storage.conn.executescript(FTS_MEMORY_SCHEMA)
        return storage
    ...
    async def record_capability(self, snapshot: CapabilitySnapshot) -> None:
    ...
    async def create_session(self, session: MemorySession) -> None:
    ...
    async def end_session(self, session_id: str, ended_at: str) -> None:
        await self.conn.execute("UPDATE memory_sessions SET ended_at=? WHERE session_id=?", (ended_at, session_id))

    async def add_observation(self, observation: MemoryObservation) -> None:
        await self.conn.execute(
    ...
    async def enqueue_observation(self, observation_id: str, now: str) -> None:
        await self.conn.execute(
    ...
    async def claim_queue(self, max_batch_size: int, max_retries: int) -> list[tuple[int, MemoryObservation]]:
    ...
    async def mark_processed(
        ...
        if row and self._fts5_enabled:
            await self.conn.execute("DELETE FROM memory_observation_fts WHERE observation_id=?", (observation_id,))
            await self.conn.execute(
    ...
    async def search_observations(
        ...
        if not self._fts5_enabled:
            return await self._search_observations_fallback(terms, workspace, limit, min_importance)
    ...
    async def _search_observations_fallback(
        self,
        terms: str,
        workspace: str,
        limit: int,
    ...
    async def get_observation(self, observation_id: str) -> MemoryObservation | None:
        async with self.conn.execute(
    ...
    async def list_sessions(self, workspace: str, limit: int = 50) -> list[MemorySession]:
        async with self.conn.execute(
    ...
    async def get_session(self, session_id: str) -> MemorySession | None:
        async with self.conn.execute(
            "SELECT session_id,workspace,project_root,started_at,ended_at,trigger_kind,command_name,metadata_json "
    ...
    async def list_citations(self, target_id: str) -> list[MemoryCitation]:
    ...
    async def status(self, workspace: str) -> dict[str, Any]:
        async with self.conn.execute("SELECT COUNT(*) FROM memory_sessions WHERE workspace=?", (workspace,)) as cursor:
            row = await cursor.fetchone()
            session_count = int(row[0]) if row else 0

        async with self.conn.execute("SELECT COUNT(*) FROM memory_observations WHERE workspace=?", (workspace,)) as cursor:
            row = await cursor.fetchone()
            observation_count = int(row[0]) if row else 0

        async with self.conn.execute("SELECT COUNT(*) FROM memory_citations WHERE workspace=?", (workspace,)) as cursor:
            row = await cursor.fetchone()
            citation_count = int(row[0]) if row else 0

        async with self.conn.execute("SELECT state, COUNT(*) FROM memory_queue GROUP BY state") as cursor:
            queue = await cursor.fetchall()
        queue_counts = {row[0]: int(row[1]) for row in queue}

        async with self.conn.execute("SELECT capability_name, available, checked_at FROM memory_capabilities ORDER BY capability_name ASC") as cursor:
